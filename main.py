...
@app.route("/adaylarr")
def adaylar():
    return render_template("adaylarr.html")
...
def vote():
    if request.method == "POST" and "selection" in request.form:
        user_id = session.get("tc_id")
        selection = request.form.get("selection")
        cursor = mysql.connection.cursor()
        
        sorgu1 = "SELECT * FROM votes WHERE user_id = %s"
        result = cursor.execute(sorgu1,(user_id,))
        #result = cursor.execute("SELECT * FROM votes WHERE user_id = %s", (user_id,))
        ...
        #    flash("Birden fazla oy kullanamazsınız.", "danger")
            ...
            sorgu = "INSERT INTO votes(user_id,selection) VALUES(%s,%s)"
            cursor.execute(sorgu,(session["tc_id"],selection))
